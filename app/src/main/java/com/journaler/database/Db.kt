...
object Db{
    ...
    val NOTE = object:Crud<Note>{
        ...
        override fun select(args: Collection<Pair<String,String>>): List<Note> {
            ...
            val cursor=db.query(true,DbHelper.TABLE_NOTES,null,selection.toString(),
                selectionArgs.toTypedArray(),null,null,null,null)
        ...
        override fun selectAll(): List<Note> {
            val db = DbHelper(name,version).writableDatabase
            val result= mutableListOf<Note>()
            val cursor=db.query(true,DbHelper.TABLE_NOTES,null,null,
    ...
    val TODO = object:Crud<Todo>{
        ...
        override fun select(args: Collection<Pair<String,String>>): List<Todo> {
            ...
            val cursor=db.query(true,DbHelper.TABLE_TODOS,null,selection.toString(),
                selectionArgs.toTypedArray(),null,null,null,null)
            while (cursor.moveToNext()){
                val id=cursor.getLong(cursor.getColumnIndexOrThrow(DbHelper.ID))
                val titleIdx=cursor.getColumnIndexOrThrow(DbHelper.COLUMN_TITLE)
                ...
                val schedulerFor = cursor.getLong(scheduledForIdx)

                val todo= Todo(title,message,location,schedulerFor)
                todo.id=id
                result.add(todo)
            }
            cursor.close()
